...
class JournalEntry(models.Model):
    ...
    voucher_type = models.CharField(
        max_length=10, choices=VOUCHER_TYPES, default="JV"
    )
    ...
    def __str__(self):
        return f"{self.voucher_no} | {self.date} | {self.get_voucher_type_display()}"

    def save(self, *args, **kwargs):
        if not self.voucher_no:  # generate only if new
            prefix = self.voucher_type  # dynamic based on voucher_type
            last_entry = (
                JournalEntry.objects.filter(voucher_no__startswith=prefix)
                .order_by("-id")
                .first()
            )
